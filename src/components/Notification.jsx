...
export default function Notification() {
  ...
  // Initial notifications data exactly matching the reference screenshots
  const [notifications, setNotifications] = useState([
    // Today Section (Screenshot 1)
    {
      id: 1,
      shopName: 'Shreeji General Store',
      initials: 'SG',
      message: 'Need 10 cases of Coca-Cola 750ml for tomorrow’s delivery.',
      time: '5 min ago',
      group: 'Today',
      isRead: false,
      isImportant: false,
    ...
    // Yesterday Section (Screenshot 2)
    {
      id: 5,
      shopName: 'Shreeji General Store',
      initials: 'SG',
  ...
  // Filter notifications by tab, time, and search query
  const filteredNotifications = notifications.filter((item) => {
    ...
    // Time filtering
    if (timeFilter === 'Today' && item.group !== 'Today') return false;
    if (timeFilter === 'Yesterday' && item.group !== 'Yesterday') return false;

    // Search query filtering
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase().trim();
      const matchName = item.shopName.toLowerCase().includes(q);
  ...
  // Group filtered notifications by day ('Today', 'Yesterday', etc.)
          ...
          <div className="max-w-7xl mx-auto space-y-6 text-left">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 pb-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {/* Time Filter Dropdown with Dropdown.svg */}
            ...
            <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-100 space-y-6">
              ...
              {groups.length > 0 ? (
                groups.map((groupName) => {
                  const groupItems = filteredNotifications.filter((n) => n.group === groupName);
