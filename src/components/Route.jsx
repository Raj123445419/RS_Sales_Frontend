            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  key={item.name}
                  type="button"
                  onClick={() => { setActiveNav(item.name); setIsMobileNavOpen(false); if (item.path && item.path !== '#' && item.path !== '/routes') navigate(item.path); }}
...
<div className="relative inline-flex items-center">
  ...
  <img
    src={Dropdown}
    alt=""
    className="absolute right-2.5 top-4.5 -translate-y-1/2 w-3 h-3 pointer-events-none"
  />
</div>
                </div>
